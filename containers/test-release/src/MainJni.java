...
/**
 * Suite de pruebas para el binding JNI (ether-sqlite-jni-runtime).
 *
 * Compilar: javac -cp ether-sqlite-jni-runtime-{VER}-fat.jar MainJni.java
 * Ejecutar: java -cp ether-sqlite-jni-runtime-{VER}-fat.jar:. MainJni
 */
public class MainJni {
    ...
    public static void main(String[] args) {
        String mode = System.getProperty("ether.sqlite.test.mode", "JNI");
        System.out.println("┌──────────────────────────────────────────────┐");
        System.out.printf( "│  ether-sqlite-jni — Release Test %-10s│%n", "[" + mode + "]");
        System.out.println("└──────────────────────────────────────────────┘");
        System.out.println();

        test("SQLite version",           MainJni::testSqliteVersion);
        test("Open memory DB",           MainJni::testOpenMemory);
        test("Open disk DB",             MainJni::testOpenDisk);
        test("DDL exec",                 MainJni::testDdl);
        test("Single insert + query",    MainJni::testInsertQuery);
        test("Batch insert",             MainJni::testBatchInsert);
        test("Named parameters",         MainJni::testNamedParams);
        test("lastInsertRowid()",        MainJni::testLastInsertRowid);
        test("changes()",                MainJni::testChanges);
        test("Auto transaction",         MainJni::testAutoTransaction);
        test("Manual transaction",       MainJni::testManualTransaction);
        test("Transaction rollback",     MainJni::testTransactionRollback);
        test("Savepoint commit",         MainJni::testSavepointCommit);
        test("Savepoint rollback",       MainJni::testSavepointRollback);
        test("WAL mode + checkpoint",    MainJni::testWal);
        test("BLOB round-trip",          MainJni::testBlob);
        test("NULL value",               MainJni::testNullValue);
        test("All numeric types",        MainJni::testNumericTypes);
        test("Column metadata",          MainJni::testColumnMetadata);
        test("columnType()",             MainJni::testColumnType);
        test("Error handling",           MainJni::testErrorHandling);
        test("Reset + clearBindings",    MainJni::testResetRebind);
        test("Multiple open/close",      MainJni::testMultipleOpenClose);
    ...
    static SqliteConnection db() { return JniSqliteConnection.memory(); }

    static void testSqliteVersion() throws Exception {
        String v = JniSqliteConnection.sqliteVersion();
    ...
    static void testOpenMemory() throws Exception {
        try (var conn = JniSqliteConnection.memory()) {
    ...
    static void testOpenDisk() throws Exception {
        File tmp = Files.createTempFile("snr-jni-disk-", ".db").toFile();
        try {
            try (var conn = JniSqliteConnection.open(tmp.getAbsolutePath())) {
    ...
    static void testInsertQuery() throws Exception {
        try (var conn = db()) {
            ...
            try (var s = conn.prepare("INSERT INTO t(name,val) VALUES(?,?)")) {
                s.bindText(1, "jni-runtime").bindDouble(2, 3.14159).stepAndDone();
            }
            try (var q = conn.prepare("SELECT id,name,val FROM t")) {
                check(q.step(), "expected a row");
                check(q.columnInt(0) == 1, "id != 1");
                check("jni-runtime".equals(q.columnText(1)), "name mismatch");
    ...
    static void testWal() throws Exception {
        File tmp = Files.createTempFile("snr-jni-wal-", ".db").toFile();
        try {
            try (var conn = JniSqliteConnection.open(tmp.getAbsolutePath())) {
                conn.enableWal();
                conn.exec("CREATE TABLE t (v INTEGER)");
