...
class SqliteConnectionTest {
    ...
    void memory_withName_opensTwoConnections() {
        ...
        try (var db1 = JniSqliteConnection.memory("test_shared");
             var db2 = JniSqliteConnection.memory("test_shared")) {
    ...
    void memory_invalidName_throws() {
        ...
        assertThrows(SqliteException.class, () -> JniSqliteConnection.memory("invalid!@#name"));
    }
    ...
    void open_fileDb(@TempDir Path tmp) throws IOException {
        ...
        try (var db = JniSqliteConnection.open(path)) {
            assertTrue(db.ping());
        }
    }
    ...
    void open_withFlags(@TempDir Path tmp) throws IOException {
        ...
        try (var db = JniSqliteConnection.open(path, SqliteLibraryJni.OPEN_READONLY)) {
            assertTrue(db.ping());
        }
    }
    ...
    void open_nonexistentParent_throws() {
        assertThrows(SqliteException.class,
            () -> JniSqliteConnection.open("/nonexistent_8f4a2b/db.sqlite"));
    }
    ...
    void open_withFlags_invalidPath_throws() {
        ...
        assertThrows(SqliteException.class,
            () -> JniSqliteConnection.open("/nonexistent_8f4a2b/db.sqlite", SqliteLibraryJni.OPEN_READONLY));
    }
    ...
    void open_withFineLogging_coversLambda(@TempDir Path tmp) throws IOException {
        ...
        var logger = Logger.getLogger(JniSqliteConnection.class.getName());
    ...
    void sqliteVersion_returnsNonEmpty() {
        var version = JniSqliteConnection.sqliteVersion();
    ...
    void ping_afterClose_throws() {
        var db = JniSqliteConnection.memory();
        db.close();
    ...
    void walCheckpoint_afterClose_throws(@TempDir Path tmp) throws IOException {
        ...
        var db2 = JniSqliteConnection.open(path);
        db2.close();
        assertThrows(SqliteException.class,
    ...
    void lastError_returnsStringOrNull() {
        ...
        assertDoesNotThrow(JniSqliteConnection::lastError);
    }
}
