...
class SqliteIntegrationTest {
    ...
    class MultiConnectionWalTest {
        ...
        void wal_writer_reader_coexist(@TempDir Path tmp) throws IOException {
            ...
            try (var writer = JniSqliteConnection.open(path)) {
                ...
                try (var reader = JniSqliteConnection.open(path)) {
        ...
        void wal_multiple_readers_no_busy(@TempDir Path tmp) throws Exception {
            ...
            try (var setup = JniSqliteConnection.open(path)) {
                setup.enableWal();
            ...
            for (int r = 0; r < READERS; r++) {
                threads.add(Thread.ofVirtual().start(() -> {
                    try {
                        barrier.await();
                        try (var conn = JniSqliteConnection.open(path)) {
