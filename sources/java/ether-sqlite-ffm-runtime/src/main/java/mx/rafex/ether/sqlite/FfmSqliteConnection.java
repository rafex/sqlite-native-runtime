 ...
 * }</pre>
 */
public final class FfmSqliteConnection implements SqliteConnection {

    // WalMode y WalCheckpointResult están definidos en la interface SqliteConnection (ether-sqlite-core)

    private static final Logger LOG = Logger.getLogger(FfmSqliteConnection.class.getName());

    private final MemorySegment handle;
    private final AtomicInteger openStatements = new AtomicInteger(0);
    private boolean closed = false;

    private FfmSqliteConnection(MemorySegment handle) {
        this.handle = handle;
    }
    ...
    public static SqliteConnection open(String path) {
        ...
        return new FfmSqliteConnection(h);
    }
    ...
    public SqliteStatement prepare(String sql) {
        ...
        return new FfmSqliteStatement(stmtHandle, openStatements::decrementAndGet);
    }
    ...
    public static String sqliteVersion() {
        return FfmSqliteStatement.readAndFreeString(SqliteLibrary.snr_sqlite_version());
    }
    ...
    public static String lastError() {
        return FfmSqliteStatement.readAndFreeString(SqliteLibrary.snr_last_error_copy());
    }
