...
class SqliteConnectionTest {
    ...
    void memory_opensAnonymous() {
        try (var db = FfmSqliteConnection.memory()) {
            assertNotNull(db);
            assertTrue(db.ping());
        }
    }
    ...
    void memory_withName_opensTwoConnections() {
        ...
        try (var db1 = FfmSqliteConnection.memory("test_shared");
             var db2 = FfmSqliteConnection.memory("test_shared")) {
    ...
    void memory_invalidName_throws() {
        ...
        assertThrows(SqliteException.class, () -> FfmSqliteConnection.memory("invalid!@#name"));
    }
    ...
    void open_fileDb(@TempDir Path tmp) throws IOException {
        ...
        try (var db = FfmSqliteConnection.open(path)) {
            assertTrue(db.ping());
        }
    }
    ...
    void open_withFlags(@TempDir Path tmp) throws IOException {
        ...
        try (var db = FfmSqliteConnection.open(path, SqliteLibrary.OPEN_READONLY)) {
            assertTrue(db.ping());
        }
    }
    ...
    void open_nonexistentParent_throws() {
        assertThrows(SqliteException.class,
            () -> FfmSqliteConnection.open("/nonexistent_8f4a2b/db.sqlite"));
    }
    ...
    void open_withFlags_invalidPath_throws() {
        ...
        assertThrows(SqliteException.class,
            () -> FfmSqliteConnection.open("/nonexistent_8f4a2b/db.sqlite", SqliteLibrary.OPEN_READONLY));
    }
    ...
    void open_withFineLogging_coversLambda(@TempDir Path tmp) throws IOException {
        ...
        var logger = Logger.getLogger(FfmSqliteConnection.class.getName());
    ...
    void sqliteVersion_returnsNonEmpty() {
        var version = FfmSqliteConnection.sqliteVersion();
    ...
    void ping_afterClose_throws() {
        var db = FfmSqliteConnection.memory();
        db.close();
    ...
    void walCheckpoint_afterClose_throws(@TempDir Path tmp) throws IOException {
        ...
        var db2 = FfmSqliteConnection.open(path);
        db2.close();
        assertThrows(SqliteException.class,
    ...
    void rawHandle_returnsNonNull() {
        try (var conn = FfmSqliteConnection.memory();
             var db   = (FfmSqliteConnection) conn) {
            var h = db.rawHandle();
            assertNotNull(h);
    ...
    void rawHandle_afterClose_throws() {
        var conn = (FfmSqliteConnection) FfmSqliteConnection.memory();
        conn.close();
    ...
    void lastError_returnsStringOrNull() {
        ...
        assertDoesNotThrow(FfmSqliteConnection::lastError);
    }
}
