...
class SqliteStatementTest {
    ...
    void rawHandle_afterClose_throws() {
        var stmt = (FfmSqliteStatement) db.prepare("SELECT 1");
        stmt.close();
    ...
    void close_withOnCloseCallback() {
        // El constructor package-private con onClose es el que usa FfmSqliteConnection.prepare()
        // snr_stmt_close(NULL) es no-op en Rust — solo verificamos que el callback se llame
        boolean[] called = {false};
        var stmt = new FfmSqliteStatement(MemorySegment.NULL, () -> called[0] = true);
        stmt.close();
    ...
    void close_withoutOnCloseCallback() {
        ...
        var stmt = new FfmSqliteStatement(MemorySegment.NULL);
        assertDoesNotThrow(stmt::close);
    }
    ...
    void lastError_doesNotThrow() {
        ...
        assertDoesNotThrow(FfmSqliteStatement::lastError);
    }
    ...
    void readInternalString_null_returnsNull() {
        assertNull(FfmSqliteStatement.readInternalString(null));
        assertNull(FfmSqliteStatement.readInternalString(MemorySegment.NULL));
    }

    @Test
    void readAndFreeString_null_returnsNull() {
        assertNull(FfmSqliteStatement.readAndFreeString(null));
        assertNull(FfmSqliteStatement.readAndFreeString(MemorySegment.NULL));
    }
}
