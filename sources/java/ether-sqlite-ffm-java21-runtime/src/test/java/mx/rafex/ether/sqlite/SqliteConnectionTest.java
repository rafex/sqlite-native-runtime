...
class SqliteConnectionTest {
    ...
    void memory_withName_opensTwoConnections() {
        ...
        try (var db1 = FfmJava21SqliteConnection.memory("test_shared");
             var db2 = FfmJava21SqliteConnection.memory("test_shared")) {
    ...
    void memory_invalidName_throws() {
        ...
        assertThrows(SqliteException.class, () -> FfmJava21SqliteConnection.memory("invalid!@#name"));
    }
    ...
    void open_fileDb(@TempDir Path tmp) throws IOException {
        ...
        try (var db = FfmJava21SqliteConnection.open(path)) {
            assertTrue(db.ping());
        }
    }
    ...
    void open_withFlags(@TempDir Path tmp) throws IOException {
        ...
        try (var db = FfmJava21SqliteConnection.open(path, SqliteLibrary.OPEN_READONLY)) {
            assertTrue(db.ping());
        }
    }
    ...
    void open_nonexistentParent_throws() {
        assertThrows(SqliteException.class,
            () -> FfmJava21SqliteConnection.open("/nonexistent_8f4a2b/db.sqlite"));
    }
    ...
    void open_withFlags_invalidPath_throws() {
        ...
        assertThrows(SqliteException.class,
            () -> FfmJava21SqliteConnection.open("/nonexistent_8f4a2b/db.sqlite", SqliteLibrary.OPEN_READONLY));
    }
    ...
    void open_withFineLogging_coversLambda(@TempDir Path tmp) throws IOException {
        ...
        var logger = Logger.getLogger(FfmJava21SqliteConnection.class.getName());
    ...
    void sqliteVersion_returnsNonEmpty() {
        var version = FfmJava21SqliteConnection.sqliteVersion();
    ...
    void walCheckpoint_afterClose_throws(@TempDir Path tmp) throws IOException {
        ...
        var db2 = FfmJava21SqliteConnection.open(path);
        db2.close();
        assertThrows(SqliteException.class,
