...
pub unsafe fn snr_open(path: *const c_char, flags: i32) -> *mut Handle {
    clear_last_error();
...
pub unsafe fn snr_open_memory(name: *const c_char) -> *mut Handle {
...
pub unsafe fn snr_close(handle: *mut Handle) {
    if !handle.is_null() {
...
pub unsafe fn snr_ping(handle: *mut Handle) -> i64 {
    clear_last_error();
...
pub fn snr_sqlite_version() -> *mut c_char {
...
pub unsafe fn snr_exec(handle: *mut Handle, sql: *const c_char) -> i32 {
    clear_last_error();
    let h = match handle_ref(handle) {
        Some(h) => h,
...
pub unsafe fn snr_last_insert_rowid(handle: *mut Handle) -> i64 {
    let h = match handle_ref(handle) {
        Some(h) => h,
        None => return 0,
    };
...
/// # Safety
/// `handle` debe ser un puntero válido.
pub unsafe fn snr_changes(handle: *mut Handle) -> i64 {
    let h = match handle_ref(handle) {
        Some(h) => h,
        None => return 0,
    };
...
/// # Safety
/// `handle` debe ser un puntero válido.
pub unsafe fn snr_set_busy_timeout(handle: *mut Handle, ms: i32) -> i32 {
    clear_last_error();
    let h = match handle_ref(handle) {
        Some(h) => h,
...
pub fn snr_flag_readonly()  -> i32 { SNR_OPEN_READONLY }
pub fn snr_flag_readwrite() -> i32 { SNR_OPEN_READWRITE }
pub fn snr_flag_create()    -> i32 { SNR_OPEN_CREATE }
