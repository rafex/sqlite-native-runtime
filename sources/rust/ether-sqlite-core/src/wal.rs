...
pub unsafe fn snr_wal_checkpoint(
    handle: *mut Handle,
...
/// # Safety
/// `handle` debe ser un puntero válido.
pub unsafe fn snr_wal_autocheckpoint(handle: *mut Handle, n: i32) -> i32 {
    clear_last_error();
    let h = match handle_ref(handle) {
        Some(h) => h,
...
pub fn snr_checkpoint_passive()  -> i32 { SNR_CHECKPOINT_PASSIVE }
pub fn snr_checkpoint_full()     -> i32 { SNR_CHECKPOINT_FULL }
pub fn snr_checkpoint_restart()  -> i32 { SNR_CHECKPOINT_RESTART }
pub fn snr_checkpoint_truncate() -> i32 { SNR_CHECKPOINT_TRUNCATE }
